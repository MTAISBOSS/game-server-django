#!/usr/bin/env python
"""
Seed script — creates superuser + initial catalog items + first season.
Run via: python scripts/seed.py
Or via Docker: docker compose run seed
"""
import os
import sys
import django
...
if not Player.objects.filter(username=ADMIN_USERNAME).exists():
    Player.objects.create_superuser(username=ADMIN_USERNAME, password=ADMIN_PASSWORD)
...
ITEMS = [
    ...
    dict(item_id='ticket_bundle_20',name='Ticket Bundle x20',description='Twenty match tickets (best value)', category='consumable', currency='coins', price=700, metadata={'quantity':20}),
]
